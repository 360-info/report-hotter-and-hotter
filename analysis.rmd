...
import_survey <- . %>%
  { read_csv(here("data", .), na = c("", "NA", "N/A")) } %>%
  clean_names() %>%
  slice(-n()) %>%
  select(-starts_with("x1")) %>%
  mutate(opt_in_date = dmy(opt_in_date))
...
test_numbers_vis %>%
  filter(experience_headache == "Yes") %>%
  {
    ggplot(.) +
      ...
      geom_col() +
      geom_richtext(
        aes(label = glue("**{scales::percent(prop)}**<br>of respondents")),
...
select_interesting_rows <- .
  select(country, opt_in_date, gender, occupation_coded, own_houe, cool_home,
